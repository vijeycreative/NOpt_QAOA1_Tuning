"""
==============================================================================
Title:             Fast Quartic and Cubic Solver (Third-Party Utility)
Subtitle:          Analytical polynomial root solvers used for QAOA parameter recovery
Source Repository: https://github.com/NKrvavica/fqs

Original Author:   NKrvavica
Original License:  MIT License
Local Use In:      Near-Optimal Parameter Tuning of Level-1 QAOA for Ising Models
Paper:             https://arxiv.org/abs/2501.16419
Project Repository:https://github.com/vijeycreative/NOpt_QAOA1_Tuning

Important Attribution Notice
----------------------------
This file is NOT original code written by V Vijendran. It is adapted from or
derived from the third-party repository:

  https://github.com/NKrvavica/fqs

The original implementation provides fast analytical solvers for cubic and
quartic polynomial equations. In this project, the code is used as a utility
for recovering stationary points of reduced QAOA objective functions, such as
when solving quartic equations that arise in analytic or semi-analytic
parameter optimization for level-1 QAOA.

Description
-----------
This module provides:
  • Closed-form solvers for single quadratic, cubic, and quartic equations.
  • Vectorized solvers for batches of quadratic, cubic, and quartic equations.
  • Lightweight wrapper functions that automatically choose between single-case
    and batched routines depending on the number of input polynomials.
  • Fast polynomial root evaluation suitable for scientific computing
    workflows where many cubic or quartic equations must be solved repeatedly.

Usage in This Project
---------------------
Within the Near-Optimal Parameter Tuning of Level-1 QAOA for Ising Models
codebase, this module is used as a numerical helper rather than as the main
scientific contribution. In particular, routines such as `single_quartic(...)`
are used to solve quartic equations that appear in the reduction of the p=1
QAOA optimization problem, allowing candidate beta stationary points to be
recovered efficiently for a fixed gamma.

Typical entry points include:
  • `single_quadratic(a0, b0, c0)`
  • `single_cubic(a0, b0, c0, d0)`
  • `single_quartic(a0, b0, c0, d0, e0)`
  • `cubic_roots(p)`
  • `quartic_roots(p)`

Examples
--------
Solve a single quartic:
    roots = single_quartic(a0, b0, c0, d0, e0)

Solve many quartics at once:
    roots = quartic_roots([
        [a0, b0, c0, d0, e0],
        [a1, b1, c1, d1, e1],
    ])
...
--------------------
• The solvers are based on closed-form analytical formulas, primarily Ferrari's
  method for quartics and Cardano's method for cubics.
• Selected functions are JIT-compiled with Numba for speed in repeated-use
  settings.
• The file may be kept locally in this repository for reproducibility and to
  avoid introducing a hard external dependency.

Citation and Credit
-------------------
Please credit the original source repository when reusing or redistributing
this file:

  NKrvavica, "fqs: Fast Quartic and Cubic Solver"
  https://github.com/NKrvavica/fqs

If you use the surrounding QAOA codebase in academic work, please cite:
